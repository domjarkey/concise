test_that("filter. mirrors dplyr::filter across input modes", {
    df <- tibble::tibble(
        x = 1:5,
    ...
    expect_equal(
        df |> filter.(context_lambda(~ is.null(z))),
    ...
    expect_equal(
        df |> filter.(~ is.null(z) ? int),
        df |> dplyr::filter(purrr::map_lgl(z, ~ is.null(.x)))
    )

    # Nested concise declarations retain only logical outputs
    expect_equal(
        df |> filter.(~ is.null(z) ? {int ; extra = 1}),
        df |> dplyr::filter(purrr::map_lgl(z, ~ is.null(.x)))
    )
})

test_that("filter. supports concise predicates with multiple inputs", {
    df <- tibble::tibble(
        x = 1:4,
    ...
    expect_equal(
        df |> filter.(~ ifelse(is.null(y), z, x > y) ? lgl),
        df |> dplyr::filter(
            purrr::pmap_lgl(
                list(x = x, y = y, z = z),
    ...
    expect_equal(
        df |> filter.(~ !is.null(y), ~ x > limit ? {limit = 1}),
        df |> dplyr::filter(!purrr::map_lgl(y, is.null)) |>
            dplyr::filter(x > 1)
    )
})

test_that("filter. pronouns work without grouping", {
...
test_that("filter. supports argument passing with ?", {
    ...
    expect_equal(
        tibble::tibble(x = 3:1) |>
            filter.(~ x + X > limit ? {lgl ; X = sum(!!(1:10)) ; limit = !!threshold}),
        tibble::tibble(x = 3:1) |>
            dplyr::filter(x + sum(1:10) > 57L)
    )
})

test_that("filter. supports recursive concise predicates", {
