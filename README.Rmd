...
N.B. Similar to `.i`/`.I`, when grouped, `.n` refers to the row index of the 
...
to make simple mappings clearer. These provide a more straightforward way of mapping
from one set to another instead of using joins and cross-walk datasets.

`%from%` and `%to%` are designed to work as a pair of ternary operators, mapping
and input vector *from* a domain vector *to* a codomain or target vector.
...
or named list as the local environment:
